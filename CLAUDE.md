...
- **`scripts/`** — Per-app installation scripts for complex setups (e.g. `install-oh-my-zsh.sh`, `install-nvm.sh`, `install-pyenv.sh`, `install-gvm.sh`).
...
- **Mandatory apps:** `iTerm2` and `Oh My Zsh` are mandatory and cannot be unselected.
- **Terminal category priority:** The `Terminal` category is always processed first and installed before any other category.
...
For complex multi-step installations (e.g. version managers that also need `~/.zshrc` configuration), prefer creating a dedicated shell script under `scripts/` and referencing it from `apps.json` via `"command": "sh scripts/<script>.sh"`.
